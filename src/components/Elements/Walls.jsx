...
const data = Data.pricePerUnit.walls
...
const Walls = (props) => {
    ...
    useEffect(() => {
      setTotal(parseFloat(ref1.current.outerText) + parseFloat(ref2.current.outerText)  + parseFloat(ref3.current.outerText) );
    });
  ...
  return (
    <>
        <div className={props.editable ? "list-container withShadows" : "list-container"}>
            ...
            <input type="number" min={0} id="walls-formar-amount" onChange={(event) => setFormar(event.target.value)} className="column-2" />
            ...
            <input type="number" min={0} id="walls-armering-amount" onChange={(event) => setArmering(event.target.value)} className="column-2"/>
            ...
            <input type="number" min={0} id="walls-betong-amount" onChange={(event) => setBetong(event.target.value)} className="column-2"/>
